...
<a href="https://github.com/endue-oss/citizenry"><img width="64" height="64" alt="Citizenry" src="https://cdn.jsdelivr.net/gh/endue-oss/.github@68968d6/logo/citizenry.svg" /></a>
...
**Federation** - instances recognizing and trusting each other on terms
each side chooses - is the protocol's north star and is in development;
the endpoints ship today behind an honest `501` (see [Roadmap](#roadmap)).
...
- `CLOUDFLARE_API_TOKEN` - scoped token. [Scoping guide](./docs/deploy.md#step-2---create-a-cloudflare-api-token).
...
1. **Sign in as admin.** Either through the admin console at
   ...
   ([Retrieving the admin password](./docs/deploy.md#step-6---retrieve-the-admin-password))
...
4. **The agent calls** `citizenry-api` (REST) using those credentials.
   (An MCP gateway, `citizenry-mcp`, is in development - see
   [Roadmap](#roadmap). REST is the supported call path today.)
...
The full surface is specified in [`packages/spec/`](./packages/spec/);
two pieces of it are still in development and tracked in the open:

- **Federation** - instance signing-key issuance, then enabling the
  gated handshake surface
  ([#3](https://github.com/endue-oss/citizenry/issues/3)). Until then
  `/.well-known/citizenry-peer` and `/federation/handshake` return
  `501`, and the instance-level `/.well-known/jwks.json` serves an
  empty key set.
- **MCP gateway** - mounting the transport and registering the
  identity/vault tools
  ([#4](https://github.com/endue-oss/citizenry/issues/4)).

Everything else above - registration, dual agent keys, per-agent
JWKS/DID discovery, the vault, mail, and the admin console - is live.
